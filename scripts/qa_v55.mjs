...
const checks=[
 ...
 ['public health version consistent', server.includes("app.get('/api/health'")&&server.includes("version:'V55'")&&server.includes('Magra School API V55')],
 ...
 ['accessible image/menu labels', jsx.includes('alt="বিদ্যালয়ের লোগো"')&&jsx.includes('aria-expanded={open}')&&jsx.includes('aria-controls="public-navigation"')&&jsx.includes('skip-link')],
 ['no duplicate React import', (jsx.match(/^import React/gm)||[]).length===1&&!jsx.split('\n').some(line=>line.trim().startsWith('mport React'))],
 ['reduced motion and focus states', css.includes('prefers-reduced-motion')&&css.includes('focus-visible')],
...
console.log(`V55 Final QA PASS: ${checks.length}/${checks.length}`);
